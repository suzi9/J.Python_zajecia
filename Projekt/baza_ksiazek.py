...
class Menu(Frame):
    def __init__(self, master):
        ...
        Label(self, text="BAZA DANYCH KSIĄŻEK", font="Times 35 bold", 
              width= 50, height= 0).pack()
        Label(self, text="Wybierz jaką akcje chcesz wykonać:", font="Times 20",
              width= 50, height= 2).pack()

        #Przyciski
        Button(self,bg="#D3D3D3", text = "Wyświetl bazę książek", height=3, 
                width=40, font = "Times 15 bold" , cursor="hand2", 
                command= lambda: master.zmiana_okna(Wyswietlanie)).pack()

        Button(self,bg="#D3D3D3", text = "Dodaj nową książke", height=3, 
                width=40, font = "Times 15 bold" , cursor="hand2", 
                command= lambda: master.zmiana_okna(Dodawanie)).pack()

        Button(self,bg="#D3D3D3", text = "Usuń książke", height=3, 
                width=40, font = "Times 15 bold" , cursor="hand2",
                command= lambda: master.zmiana_okna(Usuwanie)).pack()

        # Prycisk który zakańcza pracę programu
        Button(self,bg="#D3D3D3",text = "Zamknij", height=3, 
                width=40, font = "Times 15 bold" , cursor="hand2", 
...
class Wyswietlanie(Frame):
    def __init__(self, master):
        ...
        Label(self, text="LISTA WSZYSTKICH KSIĄŻEK W BAZIE", font="Times 20 bold", 
               width= 50, height= 0).pack()
        ...
        #Przyciski
        Button(self, bg="#D3D3D3",text = "Powrót do strony głównej", height=3, 
                width=40, font = "Times 15 bold" , cursor="hand2", 
...
class Dodawanie(Frame):
    def __init__(self,master):
        ...
        Label(self, text="DODAJ NOWĄ KSIĄŻKĘ", font="Times 35 bold", 
               width= 50, height= 0).pack()
        ...
        def dodaj_ksiazke(wprowadz_id, wprowadz_tytul, wprowadz_autora, wprowadz_rok):
            ...
            if identyfikator not in otwarcie_bazy['ID'].values:
                ...
                # używam escapechar i quoting aby do pliku .csv zapisywac dane bez 
                # cudzysłowów, żeby mi się poprawnie baza wyświetlała
        ...
        # Aby móc dodać dwa przyciski obok siebie tworzę nowy Frame w tym samym okienku i 
        ...
        Button(self,bg="#D3D3D3", text="Zapisz", fg="#3da063",cursor="hand2", width=20,
                height=3 ,font= "Times 10 bold" , 
                command=lambda:dodaj_ksiazke(wprowadz_id, wprowadz_tytul,
                 wprowadz_autora, wprowadz_rok)).pack(side=LEFT, in_=top)

        Button(self,bg="#D3D3D3", text="Wyczyść",fg="#4700ff",cursor="hand2" , 
                width=20, height=3 ,font= "Times 10 bold" , 
                command=lambda:wyczysc_dane(wprowadz_id, wprowadz_tytul,
                wprowadz_autora, wprowadz_rok)).pack(side=RIGHT, in_=top)

        Button(self,bg="#D3D3D3", text = "Powrót do strony głównej", height=4, 
                width=40, font = "Times 15 bold" , cursor="hand2", 
                command= lambda: master.zmiana_okna(Menu)).pack()
...
class Usuwanie(Frame):
    def __init__(self, master):
        ...
        Label(self, text="USUŃ KSIĄŻKĘ", font="Times 35 bold",
               width= 50, height= 0).pack()

        Label(self, text="Wprowadz ID książki którą chcesz usunąć",
               font="Times 13").pack()
        ...
        def usuwanie_ksiazki(wprowadz_id_ksiazki):
            ...
            if id_ksiazki in identyfikator.values:
                ...
                # Jeżeli podany identyfikator książki nie istnieje w bazie danych to 
                # użytkownik dostanie powiadomienie o tym aby podać istniejący identyfikator
                messagebox.showwarning("Uwaga!",
        ...
        #Przyciski
        Button(self, bg="#D3D3D3",text="Usuń", fg="#cf0000", cursor="hand2", 
        ...
        Button(self, bg="#D3D3D3",text="Wyświetl bazę po usunięciu książki", cursor="hand2", 
                width=40, height=3 ,font= "Times 15 bold",
                command= lambda: master.zmiana_okna(Wyswietlanie)).pack()
