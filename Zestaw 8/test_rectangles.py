...
class TestRectangle(unittest.TestCase): 
    ...
    def test_properties_rectangle(self):
        rec = rn.Rectangle(4, 3, 8, 9)
        assert rec.top == 9
        assert rec.left == 4
        assert rec.bottom == 3
        assert rec.right == 8
    ...
    def test_properties_points_rec(self):
        rec = rn.Rectangle(2,2, 6, 6)
        assert rec.topleft == rn.Point(2, 6)
        assert rec.topright == rn.Point(6, 6)
        assert rec.bottomleft == rn.Point(2, 2)
        assert rec.bottomright == rn.Point(6, 2)

    def test_frompoints(points):
        assert str(rn.Rectangle.from_points([rn.Point(3, 4), rn.Point(7, 8)])) == '[(3, 4), (7, 8)]'
        assert str(rn.Rectangle.from_points([rn.Point(6, 6), rn.Point(12, 10)])) == '[(6, 6), (12, 10)]'
