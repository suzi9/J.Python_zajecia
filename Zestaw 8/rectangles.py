...
class Rectangle:
    ...
    @classmethod
    def from_points(gof, points):
        if not isinstance(points, (list, tuple)):
            raise TypeError("Punkty muszą być listą lub krotką")
        
        if len(points) != 2:
            raise ValueError("Muszą być podane dwa punkty aby stworzyć prostokąt")
        
        em = gof(points[0].x, points[0].y, points[1].x, points[1].y)
